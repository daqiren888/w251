...
parser.add_argument('-b', '--batch-size', default=256, type=int, metavar='N', help='mini-batch size (default: 256), this is the total''batch size of all GPUs on the current node when'
...
parser.add_argument('--multiprocessing-distributed', action='store_true', help='Use multi-processing distributed training to launch '
                         'N processes per node, which has N GPUs. This is the ' 'fastest way to use PyTorch for either single node or '
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if not torch.cuda.is_available():
        ...
        if args.gpu is not None:
            ...
            print('args.batch=', args.batch_size )
            args.workers = int((args.workers + ngpus_per_node - 1) / ngpus_per_node)
            print('args.workers=', args.workers )
            print('model is', model)            
            model = torch.nn.parallel.DistributedDataParallel(model, device_ids=[args.gpu])
            print('model ready args.gpc=', args.gpu)
        else:
            print('gpu OK')
            model.cuda()
...
class AverageMeter(object):
    ...
    def all_reduce(self):
        total = torch.FloatTensor([self.sum, self.count]).to(0)
